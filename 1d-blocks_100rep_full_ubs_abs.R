...
for(iter in 1:100)
{
  ...
  nbasis <- max(30, ceiling(20 * n^(2/9)))
  ...
  ###
  t0=proc.time()
  my.ubs <- ssanova(my.y~my.x, id.basis=my.sample0)
  t.ubs=proc.time()-t0
  
  t0=proc.time()
  my.abs <- ssanova(my.y~my.x, id.basis=my.sample)
  t.abs=proc.time()-t0
...
write.csv(mse.compare, 'mse_1d_blocks_100rep_full_ubs_abs.csv', row.names=F)
